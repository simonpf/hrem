"""Model classes for radiative transfer emulation.

This module contains neural network models for emulating 3D radiative transfer processes.
"""
...
class RTEmulatorBase(nn.Module):
    """
    Base class for radiative transfer emulator models.
    """
...
class HeatingRateUNet3d(RTEmulatorBase):
    """
    3D U-Net for predicting vertical profiles of heating rates.
...
class HeatingRateUNet3d2(RTEmulatorBase):
    """
    ...
    def __init__(
        ...
        self.encode1 = self._make_stage(32, 48, 1, 2.0, (2, 2, 2), True)
        self.encode2 = self._make_stage(48, 64, 1, 4.0, (5, 2, 3), False)
        self.encode3 = self._make_stage(64, 128, 1, 6.0, (5, 1, 5), False)

        self.up3 = nn.Upsample(scale_factor=(5, 1, 5), mode="trilinear")
        self.decode3 = self._make_stage(128 + 64, 64, 1, 4.0, None, False)

        self.up2 = nn.Upsample(scale_factor=(5, 2, 3), mode="trilinear")
        self.decode2 = self._make_stage(64 + 48, 48, 1, 2.0, None, True)

        self.up1 = nn.Upsample(scale_factor=(2, 2, 2), mode="trilinear")
        self.decode1 = self._make_stage(48 + 32, 32, 1, 2.0, None, True)
...
class HeatingRateUNet3d3(RTEmulatorBase):
    """
    3D ResNeXt network for predicting vertical profiles of heating rates.
